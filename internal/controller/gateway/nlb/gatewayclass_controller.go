...
// GatewayClassReconciler accepts every GatewayClass whose ControllerName is the
// NLB one, writing only the Accepted condition.
...
func (r *GatewayClassReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if string(gc.Spec.ControllerName) != consts.GatewayClassControllerNameNLB {
		return ctrl.Result{}, nil
	}
	...
	accepted := metav1.Condition{
		...
		Message:            "GatewayClass accepted by " + consts.GatewayClassControllerNameNLB,
...
func (r *GatewayClassReconciler) SetupWithManager(_ context.Context, mgr manager.Manager) error {
	matches := func(o client.Object) bool {
		...
		return ok && string(gc.Spec.ControllerName) == consts.GatewayClassControllerNameNLB
	}
	return ctrl.NewControllerManagedBy(mgr).
		Named("gatewayclass-nlb").
